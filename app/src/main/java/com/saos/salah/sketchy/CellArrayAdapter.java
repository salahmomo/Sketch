...
public class CellArrayAdapter extends ArrayAdapter<String> {
    ...
    public CellArrayAdapter(Context context, int resource) {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (cell == null) {
                cell = (ViewGroup) inflater.inflate(resourceId, parent, false);
            }

            ImageView image = (ImageView) cell.findViewById(R.id.imageView);

            //(new DownloadImageTask(image)).execute(urls.get(position));
            //Picasso.with(context).load(urls.get(position)).into(image);
            MainActivity.imageLoader.displayImage(urls.get(position),image , MainActivity.displayImageOptions);
            return cell;
        }
